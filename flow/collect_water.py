...
def collect_water_use(data=None, level=5, regions=3):
    ...
    if target_types.shape[1] != 13:
        m = 'Input water flow target parameter data does not have correct number of columns. Expects 13 columns.'
        raise ValueError(m)
                    ...
                    for x4 in split_dict[x1][x2][x3]:  # level 4 target
                        for x5 in split_dict[x1][x2][x3][x4]:  # level 5 target
                            for m_type in split_dict[x1][x2][x3][x4][x5]:  # movement type
                                ...
                                if m_type == 'source':
                                    # build source name
                                    l1_total_name = f'{x1}_mgd'
                                    l1_total_value = 0
                                    l2_total_name = f'{x1}_{x2}_mgd'
                                    l2_total_value = 0
                                    l3_total_name = f'{x1}_{x2}_{x3}__mgd'
                                    l3_total_value = 0
                                    l4_total_name = f'{x1}_{x2}_{x3}_{x4}_mgd'
                                    l4_total_value = 0
                                    l5_total_name = f'{x1}_{x2}_{x3}_{x4}_{x5}_mgd'
                                    l5_total_value = 0
                                        ...
                                        for s2 in split_dict[x1][x2][x3][x4][x5][m_type][s1]:
                                            ...
                                            for s3 in split_dict[x1][x2][x3][x4][x5][m_type][s1][s2]:
                                                ...
                                                for s4 in split_dict[x1][x2][x3][x4][x5][m_type][s1][s2][s3]:
                                                    ...
                                                    l4_s_value = 0
                                                    for s5 in \
                                                            split_dict[x1][x2][x3][x4][x5][m_type][s1][s2][s3][s4]:
                        ...
                        for x5 in split_dict[x1][x2][x3][x4]:  # level 5 target
                            for m_type in split_dict[x1][x2][x3][x4][x5]:  # movement type
                                ...
                                if m_type == 'discharge':
                                    ...
                                    l1_total_name = f'{x1}_mgd'
                                    l2_total_name = f'{x1}_{x2}_mgd'
                                    l3_total_name = f'{x1}_{x2}_{x3}_mgd'
                                    l4_total_name = f'{x1}_{x2}_{x3}_{x4}_mgd'
                                    l5_total_name = f'{x1}_{x2}_{x3}_{x4}_{x5}_mgd'
                                                        ...
                                                        if l5_total_name in l5_dict:
                                                            ...
                                                            for p in \
                                                                ...
                                                                if frac_name in df.columns:
                                                                    frac = df[frac_name]
                                                                # otherwise use parameter input
                                                                else:
                                                                    frac = split_dict[x1][x2][x3][x4][x5][m_type][
                                                                        s1][s2][s3][s4][s5][p]
